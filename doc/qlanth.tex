...
It is worthwhile remembering here the spectroscopic hierarchy of descriptive elements: \index{term}\textbf{terms} correspond to $\LSbasis$ (also noted as $\LSterm{2S+1}{L}$), \index{level} \textbf{levels} correspond to $\LSJbasis$ (also noted as $\LSJterm{2S+1}{L}{J}$), \index{state} \textbf{states} correspond to $\LSJMbasis$ (also noted as $\LSJMstate{2S+1}{L}{J}{\Msub{J}}$). \figuref{fig: terms-level-states} shows an example of the relationship between a term and its associated levels and states.

\begin{wrapfigure}{r}{0.6\textwidth}
\centering
	\includegraphics[width=0.6\textwidth]{term-level-state.pdf}
...
\begin{figure}[h!]
\begin{center}
	\includegraphics[width=0.8\textwidth]{cfpGraph.pdf}
\end{center}
...
For concreteness \figuref{fig:Pr_in_LaF3} shows a depiction of a matrix representation of this Hamiltonian for the $\forb^2$ configuration.
...
\subsection{$\hamSpinSpinSpinOtherOrbitSymbol$: \spinspin and \soo}
    ...
    \index{spin-spin} With these, the expression for the \spinspin term is \cite{judd_intra-atomic_1968}
    \begin{multline}
    \ham_{s:s} = -2 \sum_{i\neq{j}}
        \sum_k \Mk{k}
            \sqrt{(k+1)(k+2)(2k+3)} 
