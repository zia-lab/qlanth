...
REFERENCES:
1. Velkov, Multi-Electron Coefficients of Fractional  Parentage  for 
   the p, d, and f Shells.
2. Nielson and Koster,  Spectroscopic  Coefficients  for the Pn, Dn, 
   and Fn Configurations.
...
Begin["`Private`"]
  ...
  HamMatrixAssembly[nf_, IiN_] := Module[
    ...
    Do[ThisEnergyMatrix[[jj, ii]] = 
      qlanth`EnergyMatrixTable[{n, AllowedJ[n][[ii]], AllowedJ[n][[jj]], IiN, 
        IiN}];,
  ...
  HoleElectronConjugation[params_] := (
    ...
    (
      ...
      flipped = Table[(flipper -> -newparams[flipper]), 
                {flipper, flipSignsOf}];
      nonflipped = Table[(flipper -> newparams[flipper]),
          {flipper, Complement[Keys[newparams], flipSignsOf]}];
      flippedParams = Association[Join[nonflipped, flipped]];
      (* newParams = Select[newParams, Not[RobustMissingQ[#]]&]; *)
  ...
  (
    ...
    numH = 14 - numE;
    numEH = Min[numE, numH];
    simpleHam = If[ValueQ[symbolicHamiltonians],
      (
